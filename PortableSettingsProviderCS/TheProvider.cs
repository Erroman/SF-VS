...
using static System.Console;
    ...
    class CustomSettingsProvider : SettingsProvider
    {
        ...
        public override SettingsPropertyValueCollection GetPropertyValues(SettingsContext context, SettingsPropertyCollection collection)
        {
            ...
            foreach (SettingsProperty setting in collection)
            {
                ...
                if (SettingsDictionary.ContainsKey(setting.Name))
                {
					...
					Console.WriteLine("if: SettingsDictionary[setting.Name].value {0}",SettingsDictionary[setting.Name].value);
                }
                else //use defaults in the case where there are no settings yet
                {
					...
					Console.WriteLine("else: SettingsDictionary[setting.Name].value {0}",SettingsDictionary[setting.Name].value);
                }
        ...
        private string UserConfigPath
        {
            get
            {
                return Settings.Default.SettingsKey; //Surely this 'SettingsKey' should be in the Settings base class,
													 //e.g. ApplicationSettingsBase
            }
