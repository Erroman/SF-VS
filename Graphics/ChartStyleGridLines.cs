...
//Добавляем оси координат, размечаем их и подписываем названия осей.
...
//Определяем отступы внутреннего холста с линиями графиков относительно наружного холста,
//на котором размещены подписи к осям координат,названия осей и значения  на метках
// determine right offset:
tb.Text = Xmax.ToString(); 
//tb.Text = "Yessssssssssssssssssssssssssssssssssssssssssssss"; 
Console.WriteLine("tb.Text = {0}", tb.Text);
tb.Measure(new Size(Double.PositiveInfinity, Double.PositiveInfinity));
Size size = tb.DesiredSize;
rightOffset = size.Width / 2 + 2; //отступ справа, по самой предположительно длинной метке
...
offset = size.Width;
}						 //находим самую длинную подпись для метки и по её длине определяем
leftOffset = offset + 5; //отступ слева

Canvas.SetLeft(ChartCanvas, leftOffset); //устанавливается отступ слева для холста с линиями графиков относительно холста с осями координат
Canvas.SetBottom(ChartCanvas, bottomOffset); //устанавливается отступ снизу для холста с линиями графиков относительно холста с осями координат
ChartCanvas.Width = Math.Abs(TextCanvas.Width - leftOffset - rightOffset);
ChartCanvas.Height = Math.Abs(TextCanvas.Height - bottomOffset - size.Height / 2); //size.Height/2 Отступ сверху на половину высоты шрифта
//Console.WriteLine("TextCanvas.Height = {0},bottomOffset = {0},TextCanvas.Height = {1},size.Height = {2}", TextCanvas.Height , bottomOffset , size.Height);

//Создаём прямоугольник, 
//стороны которого служат координатными осями на холсте
...
tick.Y1 = pt.Y;
tick.X2 = pt.X;
//убавляем 5 пунктов для получения линии метки, потому что работаем в экранных координатах!
tick.Y2 = pt.Y - 5;
...
Canvas.SetRight(tb, ChartCanvas.Width - 80);
